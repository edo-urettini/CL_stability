...
class NGPlugin(SupervisedPlugin):
    ...
    '''''
    #Update rho for Levenberg-Marquardt algorithm
    def after_update(self, strategy, **kwargs):

        with torch.no_grad(): 
            #Compute the actual improvement in the model loss
            model = copy.deepcopy(strategy.model)
            loss_after_update = torch.nn.CrossEntropyLoss()(model(strategy.mb_x), strategy.mb_y)
...
#########################################################################à
###CrossEntropyLoss that gives more weight to the buffer samples
import torch.nn.functional as F

class WeightedCrossEntropyLossPlugin(SupervisedPlugin):
    """
    WeightedCrossEntropyLossPlugin
    Custom loss plugin that assigns more weight to the second part of the batch
    proportional to the number of old classes encountered.

    The weight adjustment is based on the classes present in the current batch.
    """
    ...
    def before_forward(self, strategy, **kwargs):
        """
        Compute the number of old classes before forward pass.
        """
        
        # Extract classes from the batch
        batch_classes = set(strategy.experience.classes_in_this_experience)
        # Update old classes with the classes from the current batch
        self.old_classes.update(batch_classes)
    ...
    def __call__(self, logits, targets):
        ...
        # Define the weights: more weight to the second part of the batch
        weights = torch.ones(batch_size, device=logits.device)
        half_batch_size = batch_size // 2

        # Apply weight to buffer samples and the new samples
        n_old = len(self.old_classes)
        weights[half_batch_size:] =  n_old / self.n_new 
